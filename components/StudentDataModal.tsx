...
interface StudentDataModalProps {
  ...
  onDataSaved?: () => void;
  defaultTab?: 'biometrics' | 'physical';
}
...
export const StudentDataModal: React.FC<StudentDataModalProps> = ({
  ...
  useEffect(() => {
    ...
    setActiveTab(defaultTab === 'vma' ? 'physical' : defaultTab);
    let isMounted = true;
              ...
              {activeTab === 'physical' && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* VMA (السرعة الهوائية) */}
                    <div className="bg-amber-50/50 dark:bg-amber-950/20 p-4 rounded-2xl border border-amber-200 dark:border-amber-800/60 col-span-1 sm:col-span-2">
                      <div className="flex items-center justify-between mb-1.5">
                        <label className="text-xs font-black text-amber-800 dark:text-amber-300">
                          {language === 'ar' ? 'السرعة القصوى الهوائية (VMA)' : 'Vitesse Maximale Aérobie (VMA)'}
                        </label>
                        <span className="text-[10px] font-black text-amber-600 dark:text-amber-400">km/h (كم/س)</span>
                      </div>
                      <input
                        type="number"
                        step="0.1"
